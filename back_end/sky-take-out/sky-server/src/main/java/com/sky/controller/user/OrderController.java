...
public class OrderController {
    ...
    @ApiOperation("查询历史订单")
    public Result<PageResult> page(UserOrdersPageQueryDTO userOrdersPageQueryDTO) {
        log.info("查询历史订单：page {}, pageSize {}, status {}", userOrdersPageQueryDTO.getPage(), userOrdersPageQueryDTO.getPageSize(), userOrdersPageQueryDTO.getStatus());
        PageResult pageResult = orderService.pageQuery4User(userOrdersPageQueryDTO);
    ...
    public Result reminder(@PathVariable("id") Long orderId){
        log.info("用户催单：订单id {}", orderId);
        orderService.remainder(orderId);
        return Result.success();
    }
}
